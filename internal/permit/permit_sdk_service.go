...
type PermitSdkService struct {
	client *permit.Client
}

// NewPermitService creates a new PermitService instance
func NewPermitSdkService(permitClient *permit.Client) *PermitSdkService {
	return &PermitSdkService{
		client: permitClient,
	}
}

// Check verifies if a user has permission to perform an action on a resource
func (s *PermitSdkService) Check(ctx context.Context, userID, action, resourceType, resourceID, tenant string) (bool, error) {
...
func (s *PermitSdkService) RequirePermission(action, resourceType string) gin.HandlerFunc {
...
func (s *PermitSdkService) RequirePermissionWithParams(action, resourceType, resourceIDParam, tenantParam string) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Get user ID from context (set by auth middleware)
		userID, exists := c.Get("userID")
		if !exists {
